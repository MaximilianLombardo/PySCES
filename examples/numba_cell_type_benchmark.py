"""
Numba-accelerated ARACNe benchmark on different cell types.

This script benchmarks the Numba-accelerated ARACNe implementation on different cell types
from the Tabula Sapiens testis dataset.
"""
...
def benchmark_numba_on_stratum(stratum, tf_list, bootstraps=2):
    """Benchmark Numba-accelerated ARACNe on a single stratum."""
    # Create ARACNe object with Numba backend
    aracne = ARACNe(bootstraps=bootstraps, backend='numba')
    ...
    try:
        ...
        return run_time, edge_count, network, True
...
def run_benchmark():
    """Run a benchmark of Numba-accelerated ARACNe on different cell types."""
    ...
    # Benchmark on each stratum
    logger.info("\nBenchmarking Numba-accelerated ARACNe on each cell type...")
    
    for cell_type, stratum in strata.items():
        logger.info(f"  Running on {cell_type} ({stratum.n_obs} cells)...")
        
        run_time, edge_count, network, success = benchmark_numba_on_stratum(
            stratum, tf_list, bootstraps
        )
        
        if success:
            logger.info(f"    {run_time:.2f} seconds, {edge_count} edges")
            
            # Calculate edges per TF
            edges_per_tf = {}
            for tf_name, regulon_data in network['regulons'].items():
                edges_per_tf[tf_name] = len(regulon_data['targets'])
            
            # Store results
            results[cell_type] = {
                'time': run_time,
                'edges': edge_count,
                'edges_per_tf': edges_per_tf,
                'success': True
            }
        else:
    ...
    for cell_type, result in results.items():
        if result['success']:
            logger.info(f"{cell_type} ({strata[cell_type].n_obs} cells):")
            logger.info(f"  Runtime: {result['time']:.2f} seconds")
            logger.info(f"  Total edges: {result['edges']}")
            
            # Print edges per TF
            logger.info("  Edges per TF:")
            for tf_name, edge_count in result['edges_per_tf'].items():
                logger.info(f"    {tf_name}: {edge_count} edges")
        else:
    ...
    # Calculate edges per second for each cell type
    for cell_type, result in results.items():
        if result['success']:
            edges_per_second = result['edges'] / result['time']
            cells = strata[cell_type].n_obs
            
            logger.info(f"{cell_type}:")
            logger.info(f"  Edges per second: {edges_per_second:.2f}")
            logger.info(f"  Seconds per 1000 cells: {(result['time'] / cells) * 1000:.2f}")
